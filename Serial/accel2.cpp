...
/* プロトタイプ宣言 */
double TIME = 0.0;			//プログラム全体の時間をカウント
...
int accelflag = 0;														//accelvibrationに使うフラグ
...
/*　アクセルを振動させる関数accelflagが0で振動なし、1で振動　*/
void accel_vibration(int RISK) {
	if (RISK == 1 && accelflag == 0) {
		accelflag = 1;
		TIME = 0.0;
	}
	if(RISK == 0 && fabs(vibration)<= 0.01){
		accelflag = 0;
	}
	if (accelflag == 0) {
		vibration = 0.0;
	...
	if (accelflag == 1) {
		vibration = sin(530.0 * 2.0 * PI * TIME) * 0.5 / 180.0 * PI;			//sin(2πft) * 振幅 * ラジアン化
	}
}
...
double getaccel(double sampling_time){                      //サンプリングタイムをもらうとアクセルの踏み込み角度を算出
    ...
    pos_cmd = pos_base + pos_adm + vibration;
...
int main(char *argv[])						// 引数として *argv[] を持ってくる
{
	...
	// 通信相手が存在するかを確認
	if (inet_pton(AF_INET, "127.0.0.1", &server.sin_addr) != true)
	{
		// 存在しなければエラー
		printf("ERROR\n");
		return 1;
	...
	// 存在するので接続
	// 用意したソケットに対して通信宛先の設定を適用
	connect(sock0, (struct sockaddr *)&server, sizeof(server));
	// TCPIP 初期設定おわり

	// 5文字送信
	int ret = -10;
	// sock0 : どこのソケットから送信しますか?
	// 文字列 : どういった文字列を送信しますか?
	// 数字 : 文字列のサイズは? (送信文字列のサイズ > ここで指定する文字列のサイズ だと, 文字列は途中までしか送信されない
	ret = send(sock0, "1,0.0\n", 20, 0);
	// buf : 文字列受信用, 受け取った文字列をとりあえずこのchar型変数に与える, 受け取った文字列サイズ > bufのサイズ だと バッファオーバーフローを起こすので注意
	char buf[2] = {};
	int count = 0;
	while (true) {
		// sock0で受信した文字列を、bufに格納, 文字列受信サイズも同時に指定する必要がある
		recv(sock0, buf, 1, 0);
		printf("%s\n", buf);
	}
	...
	if (Ret != -1) {
		...
		while (1)
		{
			accel_vibration(1);							//振動させるかを入力1/0
